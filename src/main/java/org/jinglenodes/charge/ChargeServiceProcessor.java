...
public class ChargeServiceProcessor extends AbstractServiceProcessor {
        ...
        if (object instanceof JingleIQ) {
            ...
            if (session != null) {
                ...
                if (credit != null && !credit.isCharged()) {
                    ...
                    log.debug("createdConsumeEndRequest: " + request.toXML());
                    credit.setCharged(true);
                    return request;
                } else {
                    log.error("No credit found for creating ConsumeEnd");
                }
            } else {
                log.error("No session found for creating ConsumeEnd");
            }
        }
        return null;
    }
    ...
    protected void handleError(IqRequest iq) {
        log.error("Failed to send ConsumeEnd: " + iq.getResult().toXML());
    }
